...
import {DEFAULT_EVENT, CLICK_TOGGLE, LABEL_OFF_TEXT_ATTRIBUTE,LABEL_ON_TEXT_ATTRIBUTE} from "../constants.js"
...
class vanillaToggleSwitch extends HTMLElement 
{
    ...
    connectedCallback() 
    {
        ...
        //html elements
        const wrapperContainer = createComponent(WRAPPER_CONTAINER)
        const slotContainer =createComponent(SLOT_CONTAINER)
        const slot_selector = document.createElement('slot')
        const labelOffContainer = createComponent(LABEL_OFF_CONTAINER)
        labelOffContainer.innerText = attributes[LABEL_OFF_TEXT_ATTRIBUTE] 
        const svgWrapper = createComponent(SVG_WRAPPER)
        const labelOnContainer = createComponent(LABEL_ON_CONTAINER)
        labelOnContainer.innerText = attributes[LABEL_ON_TEXT_ATTRIBUTE]
        //svg elements
        ...
        this.shadow.appendChild(wrapperContainer)
            wrapperContainer.appendChild(slotContainer)
                slotContainer.appendChild(slot_selector)
            wrapperContainer.appendChild(labelOffContainer)
            wrapperContainer.appendChild(svgWrapper)
                svgWrapper.appendChild(this.svg)
                    this.svg.appendChild(this.rectOff)
                    this.svg.appendChild(this.circleOff)
            wrapperContainer.appendChild(labelOnContainer)
...
const regularToggleSwitchOnCustomComponent = 'component-regular-switch-label-on'
if (customElements.get(regularToggleSwitchOnCustomComponent) === undefined) customElements.define(regularToggleSwitchOnCustomComponent, class extends vanillaToggleSwitch {});

export const createLabelOnSwitch = function ()
{
    const regularToggleSwitch = document.createElement(regularToggleSwitchOnCustomComponent)
