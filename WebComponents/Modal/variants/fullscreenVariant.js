...
import {widthOneColumn, GRID, LAYOUT_MODAL_FULLSCREEN,POSITION_ICON_CLOSE_FULLSCREEN, PADDING_MODAL } from './constants.js'
...
const calculateWidthF = function () {
    ...
    const widthModal = LAYOUT_MODAL_FULLSCREEN.nCol[value] * width1Col + LAYOUT_MODAL_FULLSCREEN.nSpace[value] * GRID.space[value]
...
const positionateIconCloseF = function () {
    ...
    if (this.dataAttributes[SIZE_ATTRIBUTE]!== value)
    {
        ...
        Object.keys(POSITION_ICON_CLOSE_FULLSCREEN).forEach(key=>{this.iconCloseWrapper.style[key] = POSITION_ICON_CLOSE_FULLSCREEN[key][value]})
    }
    return;

}
export const fullscreenVariant = function ()
{
    ...
    this.shadow.appendChild(wrapper)
            wrapper.appendChild(this.iconCloseWrapper)
